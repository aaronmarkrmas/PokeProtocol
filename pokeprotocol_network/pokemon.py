...
def load_pokemon(filename):
    """Load Pokémon stats from the actual CSV format"""
    pokemon_data = {}
    try:
        with open(filename, 'r', encoding='utf-8') as file:
            ...
            for row in reader:
                try:
                    ...
                    type2 = row['type2'] if 'type2' in row else None
                    if type2 and type2.lower() in ['', 'nan', 'none']:
                        type2 = None
                    
                    pokemon = Pokemon(
                        name=row['name'],
                        hp=int(float(row['hp'])),  # Handle potential float values
                    ...
                    pokemon_data[row['name'].lower()] = pokemon
